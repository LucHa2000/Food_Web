...
class CartController {
  ...
  removeCart(req, res, next) {
    if (req.session.cart.totalProduct == 1) {
      ...
      let deleteProductAndTotalPrice = Cart.delete(
        req.params.id,
        req.session.cart.products
      );
      req.session.cart.totalPrice =
        req.session.cart.totalPrice - deleteProductAndTotalPrice; //  deleteProductAndTotalPrice
  ...
  updateQtyCart(req, res, next) {
    Product.findById(req.params.id).then((product) => {
      if (req.body.quantity > product.quantity) {
        ...
        let priceOld = Cart.updatePrice(
          req.params.id,
          req.session.cart.products
        );
        let newPrice = Cart.updateQtyCart(
          req.params.id,
          req.session.cart.products,
          req.body.quantity
        );
