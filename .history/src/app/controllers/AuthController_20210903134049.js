            ...
            .then((accounts) => {
                if (accounts.password == req.body.password) {
                    if (req.body["g-recaptcha-response"]) {
                        ...
                        res.cookie('userId', accounts.email)
                        res.cookie('userId', accounts.full_name)
                        if (accounts.accountType === 1) {
                            res.redirect('/')
                        } else {
                            res.locals.userName = "havietluc"
                            res.locals.email = "danchoiphonui27@gmail.com"
                            res.render('admin/site_admin')
                        }
                    } else {
                        res.render("auth/login", {
